...
public class MembershipManager {
    ...
    // public void saveMembers(String fileName, ArrayList<Member> parts) throws FileNotFoundException {
    //   File fw = new File(fileName);
    //   PrintWriter output = new PrintWriter(fw);
      
    //   for(Member sumn:parts) {
    //       output.println(sumn);
    //   }
    //   output.close();
    // }
      ...
      try (BufferedWriter writer = new BufferedWriter(new FileWriter("members.txt"))) {
        for (Member member : members.values()) {
          String line = String.format("%s; %s; %s; %s; $%.2f",
              member.getName(), member.getBirthday(),member.getPhoneNumber(), member.getMembershipType(), member.getFee());
